...
use crate::storage::{
    normalize_range, ArenaStorage, ArrayLayout, Capacity, InlineStorage, Storage,
};
...
impl<S: Storage<ArrayLayout<u8>>, I: Capacity> Clone for FromUtf8Error<S, I>
where
    Vec<u8, S, I>: Clone,
{
    fn clone(&self) -> Self {
...
impl<S: Storage<ArrayLayout<u8>>, I: Capacity> String<S, I> {
    ...
    ///
    /// let sparkle_heart = coca::InlineString::from_utf8(bytes).unwrap();
    /// assert_eq!(sparkle_heart, "💖");
    ///
    /// // Invalid Bytes:
    /// let mut bytes = coca::collections::InlineVec::<u8, 8>::new();
    /// bytes.extend_from_slice(&[0, 159, 146, 150]);
    /// assert!(coca::InlineString::from_utf8(bytes).is_err());
    /// ```
    #[inline]
    ...
    pub unsafe fn from_raw_parts(buf: S, length: I) -> Self {
        String {
            vec: Vec::from_raw_parts(buf, length),
        }
    }
    ...
    /// bytes.extend_from_slice(&[240, 159, 146, 150]);
    ///
    /// let sparkle_heart = unsafe { coca::InlineString::from_utf8_unchecked(bytes) };
    /// assert_eq!(sparkle_heart, "💖");
    /// ```
    #[inline]
    ...
    pub fn extend_from_within<R: RangeBounds<usize>>(&mut self, src: R) {
        self.try_extend_from_within(src)
            .expect("space remaining in string is insufficient");
    }
    ...
    /// # Examples
    /// ```
    /// let mut s = coca::InlineString::<3>::new();
    ///
    ...
    pub fn try_insert_str(&mut self, idx: usize, string: &str) -> crate::Result<()> {
        assert!(self.is_char_boundary(idx));
        self.vec
            .try_insert_slice(I::from_usize(idx), string.as_bytes())
    }
...
impl<S: Storage<ArrayLayout<u8>>, I: Capacity> Clone for String<S, I>
where
    Vec<u8, S, I>: Clone,
{
    #[inline]
...
impl<S1: Storage<ArrayLayout<u8>>, I1: Capacity, S2: Storage<ArrayLayout<u8>>, I2: Capacity>
    PartialEq<String<S2, I2>> for String<S1, I1>
{
    #[inline]
...
impl<S1: Storage<ArrayLayout<u8>>, I1: Capacity, S2: Storage<ArrayLayout<u8>>, I2: Capacity>
    PartialOrd<String<S2, I2>> for String<S1, I1>
{
    #[inline]
...
impl<'a, I: Capacity> crate::ArenaString<'a, I> {
...
impl<I: Capacity> String<crate::storage::AllocStorage<ArrayLayout<u8>>, I> {
    ...
    pub fn with_capacity(capacity: I) -> Self {
        Self::from(crate::storage::AllocStorage::with_capacity(
            capacity.as_usize(),
        ))
    }
    ...
    /// # Examples
    /// ```
    /// let s = coca::AllocString::try_from_str_with_capacity("abcde", 8usize).unwrap();
    ...
    pub fn from_str_with_capacity(string: &str, capacity: I) -> Self {
        Self::try_from_str_with_capacity(string, capacity)
            .expect("given string is longer than capacity")
    }
}
