...
impl<K, V, S: Storage<ListMapLayout<K, V>>, I: Capacity> ListMap<K, V, S, I> {
        ...
        K: Eq,
    {
        self.try_entry(key)
            .ok()
            .expect("map is already at capacity")
    }
...
impl<K: Debug, V: Debug, S: Storage<ListMapLayout<K, V>>, I: Capacity> Debug
    for ListMap<K, V, S, I>
{
...
impl<K: Clone, V: Clone, I: Capacity, const N: usize> Clone
    for ListMap<K, V, InlineStorage<K, V, N>, I>
{
    fn clone(&self) -> Self {
        let buf = unsafe {
            InlineStorage {
...
impl<K, V, S: Storage<ListMapLayout<K, V>>, I: Capacity> ExactSizeIterator
    for Iter<'_, K, V, S, I>
{
}
impl<K, V, S: Storage<ListMapLayout<K, V>>, I: Capacity> FusedIterator for Iter<'_, K, V, S, I> {}

impl<'a, K, V, S: Storage<ListMapLayout<K, V>>, I: Capacity> IntoIterator
    for &'a ListMap<K, V, S, I>
{
    type Item = (&'a K, &'a V);
        ...
        while front < self.map.len() {
            unsafe {
                ...
                if (self.should_remove)(k, v) {
                    ...
                    if front < new_len {
                        core::ptr::copy_nonoverlapping(
                            keys_ptr.add(new_len),
                            keys_ptr.add(front),
                            1,
                        );
                        core::ptr::copy_nonoverlapping(
                            values_ptr.add(new_len),
                            values_ptr.add(front),
                            1,
                        );
                    }
...
impl<K, V, S: Storage<ListMapLayout<K, V>>, I: Capacity, F: FnMut(&K, &mut V) -> bool> FusedIterator
    for DrainFilter<'_, K, V, S, I, F>
{
}
