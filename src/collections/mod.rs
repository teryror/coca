...
use cache::{CacheTable, LruCache2, UnitCache};
use deque::Deque;
...
/// # assert!(test().is_some());
/// ```
pub type ArenaDirectMappedCache<'src, K, V, H> =
    CacheTable<K, V, ArenaStorage<'src, ArrayLayout<UnitCache<K, V>>>, UnitCache<K, V>, H>;
...
/// # assert!(test().is_some());
/// ```
pub type Arena2WayLruCache<'src, K, V, H> =
    CacheTable<K, V, ArenaStorage<'src, ArrayLayout<LruCache2<K, V>>>, LruCache2<K, V>, H>;
...
/// # }
/// ```
pub type InlineDirectMappedCache<K, V, H, const N: usize> =
    CacheTable<K, V, InlineStorage<UnitCache<K, V>, N>, UnitCache<K, V>, H>;
/// A 2-way set-associative cache with a least recently used eviction policy,
...
/// # }
/// ```
pub type Inline2WayLruCache<K, V, H, const N: usize> =
    CacheTable<K, V, InlineStorage<LruCache2<K, V>, N>, LruCache2<K, V>, H>;
...
#[cfg(feature = "alloc")]
#[cfg_attr(docs_rs, doc(cfg(feature = "alloc")))]
pub type AllocDirectMappedCache<K, V, H> = CacheTable<
    K,
    V,
    crate::storage::AllocStorage<ArrayLayout<UnitCache<K, V>>>,
    UnitCache<K, V>,
    H,
>;
...
#[cfg(feature = "alloc")]
#[cfg_attr(docs_rs, doc(cfg(feature = "alloc")))]
pub type Alloc2WayLruCache<K, V, H> = CacheTable<
    K,
    V,
    crate::storage::AllocStorage<ArrayLayout<LruCache2<K, V>>>,
    LruCache2<K, V>,
    H,
>;
...
/// ```
pub type ArenaListMap<'a, K, V, I = usize> =
    ListMap<K, V, ArenaStorage<'a, ListMapLayout<K, V>>, I>;
...
#[cfg(feature = "alloc")]
#[cfg_attr(docs_rs, doc(cfg(feature = "alloc")))]
pub type AllocListMap<K, V, I = usize> =
    ListMap<K, V, crate::storage::AllocStorage<ListMapLayout<K, V>>, I>;
...
/// # assert!(map.is_empty());
/// ```
pub type InlineListMap<K, V, const N: usize, I = usize> =
    ListMap<K, V, list_map::InlineStorage<K, V, N>, I>;
...
/// ```
pub type DirectInlinePool<T, const N: usize, H = DefaultHandle> =
    DirectPool<T, pool::direct::InlineStorage<T, H, N>, H>;
...
#[cfg(feature = "alloc")]
#[cfg_attr(docs_rs, doc(cfg(feature = "alloc")))]
pub type PackedAllocPool<T, H = DefaultHandle> =
    PackedPool<T, crate::storage::AllocStorage<PackedPoolLayout<T, H>>, H>;
...
/// assert!(pool.is_empty());
/// ```
pub type PackedInlinePool<T, const N: usize, H = DefaultHandle> =
    PackedPool<T, pool::packed::InlineStorage<T, H, N>, H>;
