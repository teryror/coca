...
use crate::storage::{ArrayLayout, Capacity, InlineStorage, Storage};
...
impl<T: Eq, S: Storage<ArrayLayout<T>>, I: Capacity> ListSet<T, S, I> {
    ...
    /// ```
    #[inline]
    pub fn difference<'a, S2: Storage<ArrayLayout<T>>, I2: Capacity>(
        &'a self,
        other: &'a ListSet<T, S2, I2>,
    ) -> Difference<'a, T> {
        Difference {
            this: self.as_slice(),
    ...
    /// ```
    #[inline]
    pub fn symmetric_difference<'a, S2: Storage<ArrayLayout<T>>, I2: Capacity>(
        &'a self,
        other: &'a ListSet<T, S2, I2>,
    ...
    /// ```
    #[inline]
    pub fn intersection<'a, S2: Storage<ArrayLayout<T>>, I2: Capacity>(
        &'a self,
        other: &'a ListSet<T, S2, I2>,
    ) -> Intersection<'a, T> {
        Intersection {
    ...
    /// ```
    #[inline]
    pub fn union<'a, S2: Storage<ArrayLayout<T>>, I2: Capacity>(
        &'a self,
        other: &'a ListSet<T, S2, I2>,
    ) -> Union<'a, T> {
        Union {
    ...
    /// ```
    #[inline]
    pub fn contains<Q: ?Sized + Eq>(&self, value: &Q) -> bool
    where
        T: Borrow<Q>,
    {
    ...
    /// ```
    #[inline]
    pub fn get<Q: ?Sized + Eq>(&self, value: &Q) -> Option<&T>
    where
        T: Borrow<Q>,
    {
    ...
    /// ```
    pub fn is_disjoint<S2: Storage<ArrayLayout<T>>, I2: Capacity>(
        &self,
        other: &ListSet<T, S2, I2>,
    ) -> bool {
    ...
    /// ```
    pub fn is_subset_of<S2: Storage<ArrayLayout<T>>, I2: Capacity>(
        &self,
        other: &ListSet<T, S2, I2>,
    ) -> bool {
    ...
    /// ```
    #[inline]
    pub fn is_superset_of<S2: Storage<ArrayLayout<T>>, I2: Capacity>(
        &self,
        other: &ListSet<T, S2, I2>,
    ) -> bool {
    ...
    pub fn insert_unique_unchecked(&mut self, value: T) -> &T {
        self.try_insert_unique_unchecked(value)
            .ok()
    ...
    /// ```
    pub fn remove<Q: ?Sized + Eq>(&mut self, value: &Q) -> bool
    where
        T: Borrow<Q>,
    {
        if let Some((idx, _)) = self
    ...
    /// ```
    pub fn take<Q: ?Sized + Eq>(&mut self, value: &Q) -> Option<T>
    where
        T: Borrow<Q>,
    {
        if let Some((idx, _)) = self
...
impl<'a, T: Clone + Eq, S: Storage<ArrayLayout<T>>, I: Capacity> Extend<&'a T>
    for ListSet<T, S, I>
{
...
impl<T, I: Capacity, const N: usize> ListSet<T, InlineStorage<T, N>, I> {
    ...
    pub fn new() -> Self {
        ListSet {
            vec: crate::collections::InlineVec::<T, N, I>::new(),
        }
    }
}
